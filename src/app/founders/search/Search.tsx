    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-cream via-white to-cream/50">
      ...
      <div className="w-full max-w-6xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-xs sm:text-sm text-ink-soft max-w-md mx-auto px-2">
            Connect with founders, mentors, freelancers, and investors across India
          </p>
        </div>

        {/* Search Bar */}
        <div className="relative max-w-2xl mx-auto mb-4 sm:mb-6 px-1 sm:px-0">
            ...
            <div className="relative flex items-center bg-white border-2 border-line/50 rounded-xl sm:rounded-2xl shadow-sm hover:shadow-md focus-within:border-teal focus-within:shadow-lg transition-all overflow-hidden">
              ...
              {q && (
                <button 
                  ...
                  className="mr-1 sm:mr-2 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-muted/10 hover:bg-muted/20 flex items-center justify-center text-muted transition-colors text-xs"
                  aria-label="Clear search"
                >
                  ✕
                </button>
              )}
              <button className="mr-1.5 sm:mr-2 px-3 sm:px-5 py-2 sm:py-2.5 bg-gradient-to-r from-teal to-teal-dark text-cream rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium hover:shadow-lg transition-all flex items-center gap-1.5">
        ...
        <div className="flex items-center justify-between mb-3 sm:mb-4 px-1">
          ...
          {filtered.length > 0 && (
            <div className="text-[10px] sm:text-xs text-muted">Showing {filtered.length} of {initialProfiles.length}</div>
          )}
        </div>
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {filtered.map((p, idx) => {
              ...
              const isHovered = false // Removed unused variable but kept structure
              return (
                <div 
                  key={p.id}
                  className="group bg-white border border-line/50 rounded-xl sm:rounded-2xl p-3.5 sm:p-5 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col relative overflow-hidden"
                >
                  ...
                  <div className="flex items-start gap-2.5 sm:gap-3 mb-2.5 sm:mb-3 relative z-10">
                    <div className="relative shrink-0">
                      <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-cream text-sm sm:text-lg font-semibold shadow-md group-hover:shadow-lg transition-all overflow-hidden`}>
                        {p.avatar_url ? <img src={p.avatar_url} alt={p.full_name} className="w-full h-full object-cover" /> : getInitials(p.full_name)}
                      </div>
                      <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-4 sm:h-4 bg-green-400 rounded-full border-2 border-white shadow-sm"></div>
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-xs sm:text-sm text-ink truncate group-hover:text-teal transition-colors">{p.full_name}</div>
                      <div className="text-[10px] sm:text-xs text-muted truncate">{p.headline || p.company || 'CoFlare Member'}</div>
                    </div>
                  </div>

                  <span className={`inline-flex items-center gap-1 text-[8px] sm:text-[10px] font-bold tracking-wider px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full border mb-2.5 sm:mb-3 self-start relative z-10 ${roleBadge.bg}`}>
                    {roleBadge.emoji} {p.role?.toUpperCase() || 'MEMBER'}
                  </span>

                  {(p.skills || []).length > 0 && (
                    <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-2.5 sm:mb-3 relative z-10">
                      {p.skills.slice(0, 3).map((s: string, i: number) => (
                        <span key={s} className={`text-[9px] sm:text-[10px] px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full border font-medium ${SKILL_COLORS[i % SKILL_COLORS.length]}`}>{s}</span>
                      ))}
                      {p.skills.length > 3 && (
                        <span className="text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full bg-cream-dark/50 text-muted">+{p.skills.length - 3}</span>
                      )}
                    </div>
                  )}

                  {p.location && (
                    <div className="flex items-center gap-1 mb-3 sm:mb-4 text-[10px] sm:text-xs text-muted relative z-10">
                  ...
                  <div className="flex gap-1.5 sm:gap-2 mt-auto relative z-10">
                    <button 
                      onClick={() => setConnected(prev => ({ ...prev, [p.id]: !prev[p.id] }))}
                      className={`flex-1 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold transition-all flex items-center justify-center gap-1 ${
                        connected[p.id] ? 'bg-teal-light text-teal-dark border-2 border-teal/30' : 'bg-gradient-to-r from-teal to-teal-dark text-cream hover:shadow-lg'
                      }`}
                    >
                    ...
                    <Link 
                      ...
                      className="px-2.5 sm:px-4 py-2 sm:py-2.5 border-2 border-line hover:border-teal text-ink-soft hover:text-teal rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold transition-all flex items-center gap-1"
                    >
          ...
          <div className="space-y-2 sm:space-y-3">
            {filtered.map((p, idx) => {
              ...
              return (
                <div 
                  key={p.id} 
                  className="group bg-white border border-line/50 rounded-xl sm:rounded-2xl p-3 sm:p-5 shadow-sm hover:shadow-md transition-all duration-300 flex items-center gap-3 sm:gap-4"
                >
                  <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-cream text-xs sm:text-sm font-semibold shadow-md shrink-0 overflow-hidden`}>
                    {p.avatar_url ? <img src={p.avatar_url} alt={p.full_name} className="w-full h-full object-cover" /> : getInitials(p.full_name)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                      <span className="font-semibold text-xs sm:text-sm text-ink truncate max-w-[100px] sm:max-w-[200px]">{p.full_name}</span>
                      <span className={`text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full border ${roleBadge.bg}`}>
                        {roleBadge.emoji} {p.role?.toUpperCase()}
                      </span>
                    </div>
                    <div className="text-[10px] sm:text-xs text-muted truncate mt-0.5">
                      {p.headline || p.company}{p.location && <span> · {p.location}</span>}
                    </div>
                  </div>
                  <div className="flex gap-1.5 sm:gap-2 shrink-0">
                    <button 
                      onClick={() => setConnected(prev => ({ ...prev, [p.id]: !prev[p.id] }))}
                      className={`px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold transition-all ${
                        connected[p.id] ? 'bg-teal-light text-teal-dark border border-teal/30' : 'bg-teal text-cream hover:bg-teal-dark'
                      }`}
                    >
                      {connected[p.id] ? '✓' : '+ Connect'}
                    </button>
                    <Link 
                      href={`${messagingPath}?to=${p.id}`}
                      className="px-2.5 sm:px-4 py-1.5 sm:py-2 border border-line text-ink-soft rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-semibold hover:border-teal hover:text-teal transition-all flex items-center gap-1"
                    >
                      <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.86 9.86 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                      </svg>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 sm:py-20">
            ...
            <p className="text-xs sm:text-sm text-muted mb-4 sm:mb-6 max-w-sm mx-auto px-4">
