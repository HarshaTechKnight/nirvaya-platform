...
function EmbeddedPost({ post }: { post: any }) {
  if (!post) return <div className="bg-cream/50 border border-line/50 rounded-xl p-3 text-xs text-muted italic text-center">Original post unavailable</div>
  return (
    <div className="bg-gradient-to-br from-cream to-cream-dark/30 border border-line/50 rounded-xl p-3">
      <div className="flex items-center gap-2 mb-2">
        <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${getGradient(post.profiles?.full_name || 'U')} flex items-center justify-center text-cream text-[9px] font-semibold shadow-sm shrink-0 overflow-hidden`}>
        ...
        <div className="min-w-0">
          <span className="font-semibold text-[11px] text-ink">{post.profiles?.full_name}</span>
          <span className="text-[8px] font-bold tracking-wider px-1 py-0.5 rounded-full bg-teal-light/60 text-teal-dark ml-1">{(post.profiles?.role || 'founder').toUpperCase()}</span>
          <span className="text-[9px] text-muted ml-1">· {timeAgo(post.created_at)}</span>
        </div>
      </div>
      <p className="text-[11px] text-ink-soft whitespace-pre-line line-clamp-3">{post.content}</p>
    </div>
  )
}
...
function CommentSection({ postId, currentUser, onClose }: { postId: string; currentUser: any; onClose: () => void }) {
  ...
  return (
    <div className="border-t border-line/20 pt-3 mt-2">
      <div className="flex items-start gap-2 mb-3">
        <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${getGradient(currentUser?.full_name || 'U')} flex items-center justify-center text-cream text-[9px] font-semibold shrink-0 overflow-hidden`}>
          {currentUser?.avatar_url ? <img src={currentUser.avatar_url} alt="" className="w-full h-full object-cover" /> : getInitials(currentUser?.full_name)}
        </div>
        <div className="flex-1 flex items-center gap-1.5">
          <input value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handlePost() }}}
            placeholder="Add a comment..." className="flex-1 bg-cream/60 border border-line/50 rounded-full px-3 py-1.5 text-xs outline-none focus:border-teal transition-all" />
          <button onClick={handlePost} disabled={!newComment.trim() || posting} className="text-xs text-teal font-semibold disabled:opacity-40 shrink-0">{posting ? '...' : 'Post'}</button>
        </div>
      </div>
      {loading ? (
        <div className="space-y-2">{['','',''].map((_,i)=><div key={i} className="flex gap-2 animate-pulse"><div className="w-7 h-7 rounded-full bg-cream-dark shrink-0"/><div className="flex-1 bg-cream-dark/50 rounded-xl h-8"/></div>)}</div>
      ) : comments.length === 0 ? (
        <p className="text-[11px] text-muted text-center py-3">No comments yet</p>
      ) : (
        <div className="space-y-2.5">
          {comments.map(c => (
            <div key={c.id} className="flex items-start gap-2">
              <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${getGradient(c.profiles?.full_name || 'U')} flex items-center justify-center text-cream text-[9px] font-semibold shrink-0 overflow-hidden`}>
                {c.profiles?.avatar_url ? <img src={c.profiles.avatar_url} alt="" className="w-full h-full object-cover" /> : getInitials(c.profiles?.full_name)}
              </div>
              <div className="flex-1 min-w-0">
                <div className="bg-cream/60 rounded-xl px-2.5 py-1.5">
                  <div className="flex items-center gap-1.5 mb-0.5"><span className="text-[11px] font-semibold text-ink">{c.profiles?.full_name}</span><span className="text-[8px] text-muted">{timeAgo(c.created_at)}</span></div>
                  <p className="text-xs text-ink-soft">{c.content}</p>
                </div>
              </div>
            </div>
          ))}
...
export default function Feed({
  ...
  async function copyLink(postId: string) {
    const url = window.location.origin + '/founders/feed/' + postId
      ...
      <div className="max-w-3xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 w-full">
        ...
        <div className="bg-white border border-line/50 rounded-xl sm:rounded-2xl p-3 sm:p-5 mb-4 sm:mb-5 shadow-sm">
          <div className="flex gap-2 sm:gap-3 mb-3 sm:mb-4">
            <div className={`w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br ${getGradient(currentUser?.full_name || 'U')} flex items-center justify-center text-cream text-xs sm:text-sm font-semibold shadow-md shrink-0 overflow-hidden`}>
              {currentUser?.avatar_url ? <img src={currentUser.avatar_url} alt="" className="w-full h-full object-cover" /> : getInitials(currentUser?.full_name)}
            </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {posts.map((post, index) => {
            ...
            return (
              <article key={post.id} 
                className="bg-white border border-line/50 rounded-xl sm:rounded-2xl overflow-hidden hover:shadow-md transition-all duration-300 w-full"
                ...
                <div className="p-3 sm:p-5">
                  ...
                  <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {!isRepost && (
                        <span className="inline-block text-[9px] sm:text-[10px] font-medium mt-1 px-1.5 sm:px-2 py-0.5 rounded bg-cream-dark/40 text-ink-soft">{post.post_type}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1 mb-2 border-b border-line/20 pb-2 sm:pb-3 flex-wrap">
                    <button onClick={() => toggleSave(post.id)} className="text-[10px] sm:text-xs text-muted hover:text-teal font-medium px-1.5 sm:px-2 py-0.5 sm:py-1 rounded hover:bg-cream transition-colors">
                      {savedPosts[post.id] ? 'Saved' : 'Save post'}
                    </button>
                    <span className="text-line text-[10px]">·</span>
                    <button onClick={() => copyLink(post.id)} className="text-[10px] sm:text-xs text-muted hover:text-teal font-medium px-1.5 sm:px-2 py-0.5 sm:py-1 rounded hover:bg-cream transition-colors">
                      Copy link
                    </button>
                    {isOwner && (
                      <>
                        <span className="text-line text-[10px]">·</span>
                        <button onClick={() => setShowDeleteConfirm(post.id)} className="text-[10px] sm:text-xs text-muted hover:text-rust font-medium px-1.5 sm:px-2 py-0.5 sm:py-1 rounded hover:bg-cream transition-colors">
                          Delete
                        </button>
                      </>
                    )}
                    ...
                    <div className="relative flex-1">
                      ...
                      {(activeReactionPicker === post.id && !myReaction) && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white border border-line rounded-2xl shadow-2xl px-1.5 sm:px-2 py-1.5 sm:py-2 flex gap-0.5 z-30" onMouseLeave={() => setActiveReactionPicker(null)}>
                          {REACTIONS.map(r => (<button key={r.key} onClick={e => { e.stopPropagation(); handleReact(post.id, r.key) }} className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl hover:bg-cream-dark flex items-center justify-center text-lg sm:text-xl hover:scale-125 transition-all" title={r.label}>{r.emoji}</button>))}
                        </div>
                      )}
                    </div>
