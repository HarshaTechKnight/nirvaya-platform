  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl border-b border-line/20 px-2 sm:px-4 lg:px-6 h-14 sm:h-16 flex items-center gap-2 sm:gap-4 sticky top-0 z-30 shadow-sm w-full">
      ...
      <div className="flex-1 flex items-center gap-2">
        ...
        <form onSubmit={handleSearch} className={`flex-1 max-w-xl ${showMobileSearch ? 'flex' : 'hidden sm:flex'}`}>
          <div className={`relative transition-all duration-300 w-full ${isSearchFocused ? 'scale-105' : 'scale-100'}`}>
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        {/* Messages */}
        <Link href={getMessagingLink()} className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl hover:bg-cream-dark/50 flex items-center justify-center transition-all relative" title="Messages">
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-rust text-cream text-[8px] font-bold rounded-full flex items-center justify-center shadow-sm">{unreadCount > 9 ? '9+' : unreadCount}</span>
          )}
        </Link>
        ...
        <div className="relative" ref={notifRef}>
          <button onClick={() => setShowNotifs(!showNotifs)}
            className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition-all relative ${showNotifs ? 'bg-teal/10 text-teal' : 'hover:bg-cream-dark/50 text-muted hover:text-ink'}`} title="Notifications">
          ...
          {showNotifs && (
            <div className="fixed sm:absolute right-2 sm:right-0 top-[52px] sm:top-12 w-[calc(100vw-16px)] max-w-[360px] sm:w-80 lg:w-96 bg-white border border-line/50 rounded-2xl shadow-2xl overflow-hidden z-50 animate-fade-up">
              <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-line/20 bg-gradient-to-r from-cream to-white">
                <div className="flex items-center gap-2">
                  ...
                  <div><div className="font-semibold text-xs sm:text-sm text-ink">Notifications</div>{unreadCount > 0 && <div className="text-[10px] text-teal font-medium">{unreadCount} unread</div>}</div>
                </div>
              </div>
              <div className="max-h-[280px] sm:max-h-[400px] overflow-y-auto scrollbar-thin">
                {recentNotifs.length === 0 ? (
                  <div className="px-5 py-10 sm:py-12 text-center"><div className="text-3xl sm:text-4xl mb-2">🔔</div><div className="text-xs sm:text-sm text-ink-soft font-medium">No notifications yet</div></div>
                ) : (
                  recentNotifs.map(n => (
                    <button key={n.id} onClick={() => { markNotifRead(n.id); setShowNotifs(false); router.push(getNotifLink()) }}
                      ...
                      <div className="flex-1 min-w-0"><div className={`text-xs sm:text-sm leading-snug ${n.is_read ? 'text-ink-soft' : 'text-ink font-medium'}`}>{n.content}</div><div className="text-[10px] sm:text-[11px] text-muted mt-1">{timeAgo(n.created_at)}</div></div>
                    </button>
                  ))
                )}
              </div>
              <Link href={getNotifLink()} onClick={() => setShowNotifs(false)} className="block px-4 sm:px-5 py-3 border-t border-line/20 text-center text-xs sm:text-sm text-teal font-medium hover:bg-cream transition-colors">View all</Link>
            </div>
          )}
        </div>
